...
@section('content')
	<form action="{{ route('admin.movie.update', $movie->id) }}" method="POST" enctype="multipart/form-data">
		...
		{{-- 動画 --}}
		@php
			$videoFileName = old('filename', $movie->filename ?? null);
			$initialMovieUrl = $videoFileName ? asset('movie/uploader/' . $videoFileName . '.mp4') : null;
		@endphp
		<input type="hidden" name="delete_movie" value="0">
		...
		<script>
			const initialMovieUrl = @json($initialMovieUrl);
			const { createApp, ref } = Vue;
			createApp({
				setup() {
					const moviePreview = ref(initialMovieUrl);
					...
					const resetPreview = () => {
						...
						errorMessage.value = '';
						resetFileInput();
						const deleteFlag = document.querySelector('input[name="delete_movie"]');
						if (deleteFlag) {
							deleteFlag.value = '1';
						}
					};
		...
		{{-- ファイル名 --}}
		<div class="d-block">
			<x-adminlte-input name="filename" label="ファイル名" label-class="required" placeholder="ファイル名" value="{{ old('filename', $movie->filename) }}"/>
			<div class="form-note-box">
...
<!-- 		<div class="d-block">
			<div class="form-group">
				...
				<div class="radio-box">
					@foreach($conversion_types as $conversion_type)
						<div class="radio-ele">
							<input type="radio" id="conversion_type_{{ $loop->iteration }}" name="conversion_type" value="{{ $conversion_type }}" {{ old('conversion_type', $movie->conversion_type) == $conversion_type ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <div class="toggle-box">
                    <label class="toggle-label" for="is_visible">
                        ...
                        <input type="checkbox" id="is_visible" name="is_visible" value="1" {{ old('is_visible', $movie->is_visible) ? 'checked' : '' }}>
                    </label>
                </div>
            </div>
            @error('is_visible')
		...
		@php
			$expiredDate = $movie->expired_at ? \Carbon\Carbon::parse($movie->expired_at)->format('Y-m-d') : null;
		@endphp
		{{-- 有効期限 --}}
		<div class="d-block period-box">
			<x-adminlte-input-date name="expired_at" label="有効期限" :config="['format' => 'YYYY-MM-DD']" value="{{ old('expired_at', $expiredDate) }}">
		...
		{{-- 台本 --}}
		<div class="d-block">
			<x-adminlte-textarea name="script" label="台本" placeholder="台本を入力してください。" rows="7">{{ old('script', $movie->script) }}</x-adminlte-textarea>
		</div>

		{{-- メモ --}}
		<div class="d-block">
			<x-adminlte-textarea name="memo" label="メモ" placeholder="公開期間や再生回数などを自由に入力してください。" rows="7">{{ old('memo', $movie->memo) }}</x-adminlte-textarea>
		</div>
