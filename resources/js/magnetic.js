...
function magneticButton(options) {
	let settings = $.extend({
		...
		class: 'magnetizing',
		attraction: 0.3, // 1 is weak, 0 is strong
		distance: 30, // magnetic area around element
...
function magneticButton_2(options) {
	let settings = $.extend({
		target: $('[data-magnetic2]'), // jQuery element
		class: 'magnetizing2',
	...
	// distance from mouse to center of target
	distanceFromMouse_2 = function ($target, mouseX, mouseY) {
		let centerX = $target.offset().left + $target.outerWidth() / 2,
			centerY = $target.offset().top + $target.outerHeight() / 2,
			pointX = mouseX - centerX,
			pointY = mouseY - centerY,
		...
		magnetize_2 = function ($this, e) {
			...
			$this.each(function () {
				let $this = $(this),
					...
					deltaX = Math.floor(centerX - mouseX) * -1 * settings.attraction,
					deltaY = Math.floor(centerY - mouseY) * -1 * settings.attraction,
					mouseDistance = distanceFromMouse_2($this, mouseX, mouseY),
					data = {target: $this, y: deltaY, x: deltaX, distance: mouseDistance};
